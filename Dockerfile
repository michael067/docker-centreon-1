...
# Install centreon
RUN yum -y install mariadb-server && systemctl start mariadb && yum -y install centreon centreon-base-config-centreon-engine centreon-installed centreon-clapi && systemctl stop mariadb
...
RUN systemctl start mariadb && sleep 5 && mysql centreon < /tmp/cbmod.sql && /usr/bin/centreon -u admin -p centreon -a POLLERGENERATE -v 1 && /usr/bin/centreon -u admin -p centreon -a CFGMOVE -v 1 && systemctl stop mariadb
